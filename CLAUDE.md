...
2. `/proposal-manager` — Decompose requirements, extract evaluation criteria, define win themes, bid/no-bid, seed compliance matrix → `working/proposal-plan.md` + `working/compliance-matrix.md`. When `submission_mechanism: web-form`, pulls section structure from the captured portal format, not the solicitation PDF.
...
10. `/proposal-storyboard` — Section-by-section narrative spine: evaluator question, required answer, claims allowed/prohibited, proof points, graphic argument, target length → `working/storyboard.md` + `working/storyboard-coverage-map.md`. Every downstream drafting skill keys off this file.
...
14. `/proposal-editor` — Editorial pass: tightens prose, removes marketing language, removes AI smell, preserves compliance/evidence/proof. Four modes (light polish, structural rewrite, executive compression, technical preservation). → `drafts/edited/` + `reviews/editorial-changes.md`
